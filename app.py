...
def tekil_sorgu_at(kategori, sorgu, api_key_val):
    ...
    except Exception as e: return kategori, [{"baslik": "⚠️ HATA", "url": "#", "snippet": str(e)}]
        ...
        with st.container(border=True):
            ...
            if st.button("📥 Eklentiden Son Görüntüyü Al (Tekil İşlem)", use_container_width=True):
                with st.spinner("Kuyruktan veri alınıyor..."):
                    ...
                    if gelen_liste:
                        # Tekil modda sadece en son gönderileni alır
    ...
    with sag_kolon:
        with st.container(border=True):
            ...
            if analiz_butonu:
                ...
                if st.session_state.eklenti_img: st.session_state.analiz_gorselleri.append(st.session_state.eklenti_img)
                ...
                try:
                    ...
                    with st.spinner("AI Sentez Motoru Çalışıyor..."):
                        for parca in generate_multi_role_synthesis_stream(icerik_listesi, base_prompt, is_danisan):
                            tam_rapor += parca; rapor_alani.markdown(tam_rapor + "▌")
                    rapor_alani.empty(); st.session_state.rapor_sonucu = tam_rapor
...
# ----------------- MOD 3: 360 DERECE RADAR & TOPLU İŞLEM -----------------
                ...
                if st.button("📥 Tüm Kuyruğu Al", use_container_width=True):
                    with st.spinner("Kuyruk boşaltılıyor..."):
                        ...
                        if gelen_liste:
                            ...
                            st.rerun()
                        else: st.warning("Kuyrukta veri bulunamadı.")
            with col2:
                if st.button("🧹 Havuzu Temizle", use_container_width=True):
                    st.session_state.vaka_havuzu = []; st.rerun()
    ...
    with sag_kolon:
        with st.container(border=True):
            ...
            # Eğer radar taraması yapıldıysa önce onu göster
            if radar_tara_butonu:
                with st.spinner("Taranıyor..."): st.session_state.radar_link_sonuclari = gelismis_coklu_hedef_taramasi(radar_urun, "", serpapi_key)
            ...
            # Toplu Rapor Sonuçlarını Listele
            if st.session_state.vaka_havuzu:
